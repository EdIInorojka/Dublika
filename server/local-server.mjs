...
async function prepareTakePreview(sourcePath, previewPath) {
  await runFfmpeg([
    '-y', '-i', sourcePath, '-vn',
    '-af', 'aformat=sample_rates=48000:channel_layouts=mono,highpass=f=70,lowpass=f=16000,adeclick=t=2.5,afftdn=nr=8:nf=-52:tn=1:gs=6,deesser=i=.16:m=.35:f=.45,acompressor=threshold=-22dB:ratio=2.2:attack=8:release=260:makeup=1.2,speechnorm=p=.90:e=2.5:c=2.2:r=.001:f=.001:m=.6,alimiter=limit=.88,loudnorm=I=-21:TP=-2:LRA=8,volume=.7',
...
async function renderProject(user, project, { refreshAudio = false } = {}) {
  ...
  recorded.forEach((segment, index) => {
    ...
    filters.push(`[${firstTakeInput + index}:a]aformat=sample_rates=48000:channel_layouts=mono,highpass=f=70,lowpass=f=16000,adeclick=t=2.5,afftdn=nr=8:nf=-52:tn=1:gs=6,deesser=i=.16:m=.35:f=.45,acompressor=threshold=-22dB:ratio=2.2:attack=8:release=260:makeup=1.2,speechnorm=p=.90:e=2.5:c=2.2:r=.001:f=.001:m=.6,alimiter=limit=.88,atrim=start=${leadIn.toFixed(3)}:end=${takeEnd.toFixed(3)},asetpts=PTS-STARTPTS,afade=t=in:st=0:d=${edgeFade.toFixed(3)},afade=t=out:st=${fadeOutAt.toFixed(3)}:d=${edgeFade.toFixed(3)},volume=.7,adelay=${delay}:all=1[t${index}]`);
  });
